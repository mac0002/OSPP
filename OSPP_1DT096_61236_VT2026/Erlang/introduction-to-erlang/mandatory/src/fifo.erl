...
%% @doc Returns a interger representing the number of elements in the FIFO.
-spec size(Fifo) -> integer() when Fifo :: fifo().
size({fifo, In, Out}) ->
    length(In) + length(Out).

%% @doc Pushes a element X to the Fifo and returns a new Fifo
...
%% @doc Pops the top element in the Fifo. Returns the
%% the popped element with a new Fifo.
%% @throws 'empty fifo'

%% pop should return {Value, NewFifo}
-spec pop(Fifo) -> {term(), fifo()} when Fifo :: fifo().
pop({fifo, [], []}) ->
...
%% Helper pop() function when In list is empty.
pop({fifo, [], [H | T]}) ->
    {H, {fifo, [], T}};
...
%% @doc Check if the Fifo is empty.
