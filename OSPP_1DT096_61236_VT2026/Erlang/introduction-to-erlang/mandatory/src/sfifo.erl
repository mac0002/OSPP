...
loop(Fifo) ->
    receive
        ...
        {empty, PID} ->
            ...
            loop(Fifo);
        {push, PID, Value} ->
            FifoNew = fifo:push(Fifo, Value),
            PID ! {push, FifoNew},
            loop(FifoNew);
        {pop, PID} ->
            try fifo:pop(Fifo) of       %% Use try/catch because pop raises an error.
                {Value, NewFifo} ->
                    PID ! {pop, {Value, NewFifo}},
                    loop(NewFifo)
            catch
                error: 'empty fifo' ->
                    PID ! {pop, {error, 'empty fifo'}},
                    loop(Fifo)
            end     % Don't need ";" or "." for some reason.
        % {stop, From} ->
        %     From ! {stop, ok}
    end.
