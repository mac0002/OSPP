...
%% @doc Prints "Hello!" to the terminal N times.
...
-spec max(L) -> M
    when L :: [integer()],
         M :: integer().
max([H | T]) ->
    F = fun (X, Y)          %% X is equivalent to a element,
        when X > Y -> X;    %% Y is equivalent to an Acc.
            (_, Y) -> Y
        end,
    lists:foldl(F, H, T).
