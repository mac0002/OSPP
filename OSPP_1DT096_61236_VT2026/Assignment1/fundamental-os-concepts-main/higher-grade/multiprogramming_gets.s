...
# Buffer size for gets
.eqv BUFFER_SIZE 6	# String buffer size with null termination included
USER_STR_BUFFER: 	.space BUFFER_SIZE	# User's string buffer
...
#------------------------------------------------------------------------------
# GETS with no gets syscall
#
# User level job. 
#------------------------------------------------------------------------------

job_gets_no_gets_syscall:
	...
	li $v0, 12
	teqi $zero, 0   	# Custom read_char system call.
...
no_sysgets_infinite_loop:
	
	li $s0, 0 		# For indexing the buffer array
...
job_get_char_loop:
	...
	# ASCII value of key pressed now in $v0
	sb $v0, USER_STR_BUFFER($s0)
	addi $s0, $s0, 1	# Increase the index value by 1 byte for 1 char
	
	bne $s0, 3, job_get_char_loop
	
	sb $zero, USER_STR_BUFFER($s0)
	
	# Use the MARS builtin system call (4) to print strings.

	la $v0, USER_STR_BUFFER		# Print the string from keyboard
	sb $v0, CUSTOM_GETS_RESULT + 17
	li $v0, 4
	la $a0, CUSTOM_GETS_RESULT	# Print the info string 
	syscall 
	
	li $v0, 4 
	la $a0, USER_STR_BUFFER		# Print the string from keyboard
	syscall
...
job_gets:
	...
	la $a0, USER_STR_BUFFER
	li $a1, BUFFER_SIZE	
		
	# Use custom gets system call (code 8)
	li $v0, 8
	teqi $zero, 0   	# Custom read_s system call.
...
__job_0_context: .space 24 	# Job increment.

__job_1_context: .space 24	# Job gets.

__user_buf_addr: .word 0	# Address of user's buffer
# __string_buf:    .space 256	# Space for characters plus a null termination.
__string_index:  .word 0	# Index for traversing the string
__buffer_limit:  .word 0 	# Determined by user
...
__system_call_gets:
	...
	# Save the size of the buffer from user code
	sw $a1, __buffer_limit
	
	# Save the address of user's buffer
	sw $a0, __user_buf_addr
	
	# Block the caller by changing the state of the caller to waiting. 
	sw $k0, __waiting
	
	# Get address of the instruction (teqi) causing the exception. 
	# and store it in $t0. 
	mfc0 $t0, $14
...
TODO_8:	# Before resuming the waiting job, put the ASCII value of the pressed
	...
	lw $t0, __string_index
	lw $t1, __user_buf_addr
	add $t1, $t1, $t0
	sb $v0, 0($t1)	# Save the character into the buffer (by indexing)
	
	addi $t0, $t0, 1		# Increase index value

	lw $k1, __buffer_limit		# Get buffer limit/size
	addi $k1, $k1, -1               # Since index starts counting from 0
	

	bne $k1, $t0, __string_not_done # Jump over String done until reaching buffer limit
	
	# String done
	# sb $zero, 0($t1)		# Add null termination
	lw $t1, __user_buf_addr         # Reload buffer address
	add $t1, $t1, $t0               # Add index 
	sb $zero, 0($t1)                # Null terminate 

	lw $v0, __user_buf_addr
	move $a0, $v0
	li $v0, 4              		# Print string syscall
	syscall
		
	# lw $v0, __user_buf_addr	# $v0 - as an output, could contain the result
	sw $zero, __string_index	# Reset the index value
...
__string_not_done:
	...
	sw $t0, __string_index		# Save the index value
	
	# Not done? Reset the job (gets) into waiting state.
	lw $k0, __running
	sw $k0, __waiting
	lw $k0, __ready
	sw $k0, __running
	
	# No job is ready anymore. 
	li $t0, -1
	sw $t0, __ready
